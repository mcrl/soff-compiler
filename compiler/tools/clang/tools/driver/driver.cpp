...
extern int cc1as_main(const char **ArgBegin, const char **ArgEnd,
                      const char *Argv0, void *MainAddr);
extern int snuclcc_main(const char **ArgBegin, const char **ArgEnd,
                        const char *Argv0, void *MainAddr);
...
int main(int argc_, const char **argv_) {
  ...
  // SnuCL Compiler
  if (argv.size() > 1 && StringRef(argv[1]).startswith("-snu")) {
    return snuclcc_main(argv.data()+2, argv.data()+argv.size(), argv[0],
